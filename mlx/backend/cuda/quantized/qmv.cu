...
template <typename T, int rows_per_block, int n_per_thread, int bits, int group_size, bool use_mx_scale>
__device__ void
qmv_impl(const uint8_t* mat, const uint8_t* scales_, const T* vec, T* out, int rows, int cols) {
  ...
  if (row < rows) {
    ...
    scales += row * (cols / group_size) + (warp.thread_rank() * nv_per_thread) / group_size;
    float sum = 0.0f;
    for (int col = n_per_thread * warp.thread_rank(); col < packed_cols;
         col += (WARP_SIZE * n_per_thread)) {
      auto local_mat =
          unsafe_load_vector<n_per_thread>(mat + row * packed_cols + col, 0);
      auto local_vec = unsafe_load_vector<nv_per_thread>(vec + vals_per_byte * col, 0);
      float local_sum = 0.0f;
#pragma unroll
      for (int j = 0; j < n_per_thread; ++j) {
        if constexpr (bits == 8) {
          local_sum += Dequantize<bits>{}(local_mat[j]) * static_cast<float>(local_vec[j]);
        } else {
          local_sum += Dequantize<bits>{}(local_mat[j]) * static_cast<float>(local_vec[2 * j]);
          local_sum += Dequantize<bits>{}(local_mat[j] >> 4) * static_cast<float>(local_vec[2 * j + 1]);
        }
      }
...
template <typename T, int rows_per_block, int n_per_thread, int bits, int group_size, bool use_mx_scale>
__global__ void
qmv_single(const uint8_t* mat, const uint8_t* scales, const T* vec, T* out, int rows, int cols) {
  qmv_impl<T, rows_per_block, n_per_thread, bits, group_size, use_mx_scale>(mat, scales, vec, out, rows, cols);
}
